import { mapDateAdapter, nativeDateAdapter } from "native-date-adapter";
...
function FlatpickrInstance(
  ...
  function setupHelperFunctions() {
    self.utils = {
      getDaysInMonth(month = self.currentMonth, yr = self.currentYear) {
        const dateAdapter = mapDateAdapter.get(<string>self.config.locale);

        if (dateAdapter) {
          return dateAdapter.daysInMonth(yr, month + 1);
        }
    ...
    if (self.selectedDates.length === 0) {
      const defaultDate =
        self.config.minDate === undefined ||
          compareDates(new dateHandler(), self.config.minDate) >= 0
          ? new dateHandler()
          : new dateHandler(self.config.minDate.getTime());
  ...
  function buildMonthDays(year: number, month: number) {
    const firstOfMonth =
      (new dateHandler(year, month, 1).getDay() -
    ...
    for (; dayNumber <= prevMonthDays; dayNumber++, dayIndex++) {
      days.appendChild(
        createDay(
          ...
          new dateHandler(year, month - 1, dayNumber),
          dayNumber,
          dayIndex
        )
      );
    ...
    for (dayNumber = 1; dayNumber <= daysInMonth; dayNumber++, dayIndex++) {
      days.appendChild(
        createDay(
          "flatpickr-day",
          new dateHandler(year, month, dayNumber),
          dayNumber,
          dayIndex
        )
      );
    ...
    for (
      ...
      days.appendChild(
        createDay(
          ...
          new dateHandler(year, month + 1, dayNum % daysInMonth),
          dayNum,
          dayIndex
        )
      );
  ...
  function buildDays() {
    ...
    for (let i = 0; i < self.config.showMonths; i++) {
      const d = new dateHandler(self.currentYear, self.currentMonth, 1);
  ...
  function buildMonthSwitch() {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      month.value = new dateHandler(self.currentYear, i).getMonth().toString();
  ...
  function onMouseOver(elem?: DayElement, cellClass = "flatpickr-day") {
    ...
    for (let t = rangeStartDate; t < rangeEndDate; t += duration.DAY) {
      if (!isEnabled(new dateHandler(t), true)) {
        containsDisabled =
  ...
  function parseConfig() {
    ...
    dateHandler = (<any>(
      nativeDateAdapter(<string>userConfig.locale)
    )) as DateConstructor;
  ...
  function setupLocale() {
    ...
    self.parseDate = createDateParser({ config: self.config, l10n: self.l10n, dateHandler });
  }
  ...
  function selectDate(e: MouseEvent | KeyboardEvent) {
    ...
    const selectedDate = (self.latestSelectedDateObj = new dateHandler(
  ...
  function setupDates() {
    self.selectedDates = [];
    self.now = self.parseDate(self.config.now) || new dateHandler();
  ...
  function getDateStr(specificFormat?: string, isAltInput = false) {
    const format =
      specificFormat ||
      (self.config.altInput ? self.config.altFormat : self.config.dateFormat);

    return self.selectedDates
      .map((dObj) => self.formatDate(isAltInput ? new Date(dObj) : dObj, format))
      .filter(
        (d, i, arr) =>
          self.config.mode !== "range" ||
          self.config.enableTime ||
  ...
  function updateValue(triggerChange = true) {
    ...
    self.input.value = getDateStr(self.config.dateFormat, true);
